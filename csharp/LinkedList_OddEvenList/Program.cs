...
internal class Program
{
    private static void Main(string[] args)
    {
        var scenarios = new[]
        {
            new Scenario("[1,2,3,4,5]",   Build(1,2,3,4,5),   "[1,3,5,2,4]"),
            new Scenario("[2,1,3,5,6,4,7]", Build(2,1,3,5,6,4,7), "[2,3,6,7,1,5,4]"),
            new Scenario("[1]",            Build(1),            "[1]"),
            new Scenario("[1,2]",          Build(1,2),          "[1,2]"),
        };
    ...
    private static void RunScenario(Scenario scenario)
    {
        ...
        foreach (var method in methods)
        {
            ...
            try
            {
                result = method.Invoke(solution, new object?[] { headCopy });
            }
